  ...
  return (
    <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-linear-to-br from-white to-gray-50">
      <div className="relative h-64">
        ...
        {/* Universe badge */}
        <Badge className={
          `absolute top-3 right-3 text-xs ${ hero.universe === 'DC' ? 'bg-blue-500' : 'bg-red-500' } text-white`
        }>{hero.universe}</Badge>
        ...
        >
          <Heart 
            className={
              `h-4 w-4 ${ isFavorite(hero) ? 'fill-red-500 text-red-500' : 'fill-gray-500 text-gray-500' }`
            } 
          />
        </Button>
      ...
      <CardHeader className="py-3 z-10 bg-gray-100/50 backdrop-blur-sm relative top-1 group-hover:-top-2.5 transition-all duration-300">
        <div className="flex justify-between items-start">
          ...
          <Badge className={
            `text-xs ${ hero.category === 'Hero' ? 'border-green-200 bg-green-100 text-green-800' : 'border-red-200 bg-red-100 text-red-800' }`
          }>{hero.category}</Badge>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <Progress value={(hero?.strength ?? 0) * 10} className="h-2" activeColor="bg-orange-500" />
          </div>
          <div className="space-y-1">
            <div className="flex items-center gap-1">
            ...
            <Progress value={(hero.intelligence ?? 0) * 10} className="h-2" activeColor="bg-blue-500"/>
          </div>
          <div className="space-y-1">
            <div className="flex items-center gap-1">
            ...
            <Progress value={(hero.speed ?? 0) * 10} className="h-2" activeColor="bg-green-500" />
          </div>
          <div className="space-y-1">
            <div className="flex items-center gap-1">
            ...
            <Progress value={(hero.durability ?? 0) * 10} className="h-2" activeColor="bg-purple-500" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {
              (hero?.powers?.length ?? 0) > 3 && (
                <Badge variant="outline" className="text-xs">
                  { (hero?.powers?.length ?? 0) - 3 } more
                </Badge>
              )
            }
          </div>
